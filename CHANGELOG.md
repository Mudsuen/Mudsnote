...
- Lesson: A knowledge graph is useful when it preserves the distinction between confirmed structure and machine inference, keeps the Markdown index canonical, and makes navigation clearer without becoming a second editor.
...
- Fix: Headings remain presentation-only; the recursive folder drawer is restored while folder taps filter the existing home timeline to that folder and all descendants. The top chrome keeps one uniform adaptive color and applies continuous native ultra-thin material from the screen edge through the former 128-point header boundary, retaining a subtle divider there so every note scrolled behind the buttons is blurred without a white opacity gradient. The drawer keeps one centered Folders title; its normal top action opens Settings, while Edit changes that action to New Folder. Independent captures use readable collision-safe names and clean Markdown without hidden write markers or timestamp headings, and iOS new notes no longer offer a per-note destination: saves always use the default folder from Settings while holding the selected library's security-scoped access during target preparation. Unchanged autosaved new notes still finalize on edit exit, native selection spans rendered paragraphs and list markers, and appearance can follow the system or stay explicitly light or dark.
