...
- Fix: Headings remain presentation-only; the recursive folder drawer is restored while folder taps filter the existing home timeline. One fading material now occupies the actual top safe-area chrome, stays translucent instead of stacking over a second black toolbar, and begins fading at the toolbar icon row. The drawer keeps one Folders title and moves Settings into the Library card. Independent captures use readable collision-safe names and clean Markdown without hidden write markers or timestamp headings, unchanged autosaved new notes still finalize on edit exit, native selection spans rendered paragraphs and list markers, and appearance can follow the system or stay explicitly light or dark.
