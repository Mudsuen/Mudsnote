...
- Lesson: A recovery queue should serve external or interrupted work, not sit in the foreground write path; persistent diagnostics and newly discovered recovery events are separate UI states.

### 260. Pro-reviewed capture integrity and bounded iOS inventory
- Problem: Distinct captures with identical text could collapse into one file, same-time attachments could reuse a path, queued work could follow a later library switch, and one damaged recovery sidecar could make the whole draft appear damaged. The Notes page also limited only rendered rows while still enumerating and parsing the full File Provider library, and restored UI behavior was weakly covered in CI.
- Fix: Pending writes now carry a stable library identity and own durable target claims and completion receipts, so replay is idempotent per operation without equating identical user content. Attachment paths are operation-unique and reject different existing bytes; queue initialization uses exclusive creation; pending notes remain visible from the durable queue; partial draft recovery restores healthy text and attachments. iOS library inventory now advances in 40-note actor-owned pages as scrolling reaches the footer, excludes Markdown under `Attachments`, and invalidates metadata caches when the underlying file identity changes. macOS standalone and floating editors now use the same conflict-preserving save contract as the library. Folder and sidebar preferences are library-scoped, selected rows expose accessibility state, the top extension uses real material, and CI requires the capture, folder-selection, and top-material UI regressions.
- Lesson: Capture identity is the operation, not its bytes or preferred filename; recovery state must stay bound to its authorized root; and pagination is real only when it bounds filesystem inventory and metadata work below the view layer.
