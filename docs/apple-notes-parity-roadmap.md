...
- Toolbar uses Notes-like unified titlebar chrome so traffic lights and controls share one row, with split-view-tracking separators aligned to the source/list/editor pane boundaries. List options remain on the note-list side while New Note sits immediately across the tracked editor divider, matching Notes ownership in expanded and collapsed states. Both use compact `30x30` native macOS 26 glass circles; editor format/checklist/table/link/attachment actions use one `184x32` `NSGlassEffectView`. The nonessential share/export and more group has been removed. Disabled state remains an efficient group-level fade with icon-level command availability, while custom CALayer fill and border rendering has been removed. List options and `Aa` use dedicated mouse-down menu triggers that stay highlighted throughout menu tracking, share a button-relative anchor, and retain a safe keyboard fallback. A wider Notes-like search field, stateful source-list toggling, restrained symbols, and native app/File/Edit/View/Window commands remain present; deeper per-symbol tuning remains open.
- Titlebar, list headers, default window proportion, and shared layout metrics now use a reference-scale `1080x720` shell whose aspect ratio closely matches the supplied `931x623pt` Apple Notes window. Default source/list columns are a compact `220/220pt`, conservatively approaching the reference's roughly `212/200pt` proportions; source/list widths, source visibility, and the whole workspace frame persist across launches. Restored frames are clamped onto the nearest visible display, while canonical visual QA ignores personal frame state. The version-3 layout migration replaces untouched `250pt` defaults but preserves manually resized panes.
- Source list is a full-height native `NSSplitViewItem.sidebar` with a darkened native material and `24pt` rounded boundary covering the titlebar, traffic lights, Add Folder, and sidebar-toggle controls in expanded mode. Its rows begin below the titlebar safe area. User collapse runs through a `0.22s` native split-item animation; the whole region and Add Folder control disappear, the sidebar control becomes a bordered circular button, and the content-list item becomes leftmost while retaining its current library title. Persisted widths, tracking separators, compact `192pt` internal rows, shared counts, deferred loading, disclosure, recently deleted, keyboard hierarchy navigation, drag targeting, and field-editor-backed inline create/rename remain intact. The non-core call-recordings source remains intentionally omitted; deeper hierarchy tuning remains open.
...
- Editor now uses a safe-area-anchored centered date-only header for loaded notes, explicit left-aligned note titles, debounced autosave that leaves the displayed timestamp unchanged while editing and refreshes it from the file modification date after a successful save, compact `24pt` Notes-like title typography, a `15pt` body reading size, restrained line rhythm, content-reference-matched `30pt` date-to-title spacing, and a `24pt` content inset close to the editor divider. Empty notes and no-selection states keep a quiet blank canvas instead of showing a custom instructional overlay; deeper content-state tuning for complex notes remains open.
...
- Visual QA has a repeatable active-window side-by-side harness with quiet source-loading rows, pointer-hover suppression before capture, explicit frontmost-app verification, physical external-display preference when one is connected, deterministic expanded/collapsed source-list state, true window-only capture normalization with bottom-to-top coordinate conversion for full-screen fallback crops, point/pixel/backing-scale/window-bounds/capture-mode metadata, explicit `2x` point metrics for the checked-in Retina references, fixture-aware routing to the matching empty or content Apple Notes reference, focus-stable requested-note selection that avoids an editor caret in comparison captures, canonical `1080x720` sizing, reference-scale shell geometry, and a compact no-rim toolbar baseline. The accepted scale comparison is stored under `docs/visual-qa/compact-scale-comparison.png`.
