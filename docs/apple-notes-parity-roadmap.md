...
- Titlebar, list headers, default window proportion, and shared layout metrics are closer to Notes, with a larger first-launch desktop window and screen-clamped presentation, but still need source-list hierarchy and exact toolbar balance tuning.
- Source list has compact group labels, wider Notes-like default column geometry, tighter Notes-like row density, true all-note counts from a shared library snapshot, warm selected-row tint, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, session-local disclosure controls, toolbar show/hide behavior, and cached drag-target validation, but still needs deeper hierarchy visual tuning.
- Note-list rows show bounded body previews, a darker Notes-like column, a true filesystem-backed all-notes scope, wider Notes-like default column geometry, shared-snapshot folder/tag filtering, centralized horizontal spacing, inset rounded selected cards, subtle hover feedback, subtle row separators, native Markdown file drag-out, drag-to-folder moves with cached validation, compact attachment indicators, local image thumbnails, tighter row density, launch-safe shell-first direct-open hydration, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still need richer multi-item drag polish.
