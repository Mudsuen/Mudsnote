...
- Toolbar uses Notes-like unified titlebar chrome so traffic lights and controls share one row, with split-view-tracking separators aligned to the source/list/editor pane boundaries. Middle-column list options and New Note use compact `30x30` native macOS 26 glass circles; editor format/checklist/table/link/attachment actions use one `184x32` `NSGlassEffectView`; copy/export and more actions use a `72x32` native glass group. Disabled state remains an efficient group-level fade with icon-level command availability, while custom CALayer fill and border rendering has been removed. Markdown copy/export, selection-aware more actions, a wider Notes-like search field, stateful source-list toggling, restrained symbols, and native app/File/Edit/View/Window commands remain present; deeper per-symbol tuning remains open.
...
- Source list is now an independent dark sidebar-material region with a `24pt` native rounded boundary in expanded mode; when collapsed, the note list becomes the leftmost content region while the unified toolbar retains the sidebar control and current library title. Inside it, `222pt` rows use `14pt` insets, a `36pt` rhythm, compact symbols/type, shared-snapshot counts, warm selection, quiet hover, deferred folder/tag loading, persisted disclosure, recently deleted, keyboard hierarchy navigation, cached drag targeting, and a field-editor-backed inline create/rename control. The non-core call-recordings source remains intentionally omitted; deeper hierarchy tuning remains open.
