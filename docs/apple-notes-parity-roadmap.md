...
- Source list has compact group labels, wider Notes-like default column geometry, stronger Notes-like typography and row height, true all-note counts from a shared library snapshot, warm selected-row tint, quiet hover feedback, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, session-local folder disclosure controls, Notes-like collapsible folder/tag sections, toolbar show/hide behavior, and cached drag-target validation, but still needs deeper hierarchy visual tuning.
