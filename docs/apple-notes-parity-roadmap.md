...
- Note-list rows show bounded body previews, a darker Notes-like column, inset rounded selected cards, subtle hover feedback, native Markdown file drag-out, drag-to-folder moves, compact attachment indicators, local image thumbnails, tighter row density, shell-first direct-open hydration, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still need side-by-side spacing tuning and richer multi-item drag polish.
...
- Visual QA has a repeatable active-window side-by-side harness with QA-only source-shell stabilization against the supplied Apple Notes screenshot, but still needs per-iteration delta notes and closer visual tuning.
