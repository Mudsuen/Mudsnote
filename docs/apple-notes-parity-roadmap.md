...
- Toolbar has a Notes-like expanded top chrome, split-view-tracking separators aligned to the source/list/editor pane boundaries, grouped editor-tools capsule for format/checklist/table/link/attachment actions with compact `184x32` plain dark-fill treatment, a hard no-rim border path when the border alpha is zero, whole-capsule disabled fade for the editor-tools group, calmer icon-level tinting for standalone menu buttons, compact `30x28` borderless icon menu buttons for share/export and more actions, Markdown export/share affordances for system sharing, copying content, exporting one file, and sharing/exporting multiple selected Markdown files, a more-actions menu with selection-count-aware file action labels, a wider Notes-like search field, stateful source-list toggling, restrained toolbar symbol sizing, and context-aware disabled states, but still needs deeper per-symbol toolbar tuning against Notes.
