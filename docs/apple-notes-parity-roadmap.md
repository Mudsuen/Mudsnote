...
- Source list has compact group labels, tighter Notes-like row density, counts, warm selected-row tint, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, and session-local disclosure controls, but still needs side-by-side spacing checks and deeper hierarchy visual tuning.
