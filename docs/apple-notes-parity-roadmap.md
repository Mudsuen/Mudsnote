...
- Toolbar uses Notes-like unified titlebar chrome so traffic lights and controls share one row, with split-view-tracking separators aligned to the source/list/editor pane boundaries, dedicated compact no-rim `30x30` dark circular buttons for middle-column list options and New Note, grouped editor-tools capsule for format/checklist/table/link/attachment actions with compact `184x32` plain dark-fill treatment, a hard no-rim border path when the border alpha is zero, whole-capsule disabled fade for the editor-tools group, a compact no-rim `72x32` file-actions capsule that groups independent Markdown copy/export and current-note more buttons, calmer icon-level tinting for menu-button disabled states, compact `30x28` borderless icon menu buttons, Markdown actions for copying content and exporting one or multiple selected files, a more-actions menu with selection-count-aware file action labels, a wider Notes-like search field, stateful source-list toggling, restrained toolbar symbol sizing, and context-aware disabled states. A lightweight native app/File/Edit/View/Window menu routes `Command-N` into a true unsaved library-note state and `Shift-Command-N` into source-list inline folder creation while quick capture keeps its separate hotkey; deeper per-symbol toolbar tuning against Notes remains open.
- Titlebar, list headers, default window proportion, and shared layout metrics now use a reference-scale `1080x720` shell whose aspect ratio closely matches the supplied `931x623pt` Apple Notes window. Default source/list columns are `250/250pt`; source/list widths, source visibility, and the whole workspace frame persist across launches. Restored frames are clamped onto the nearest visible display, while canonical visual QA ignores personal frame state. A one-time layout-scale migration clears stored oversized pane widths while preserving later user resizing.
- Source list uses `222pt` rows inside `14pt` column insets, a `36pt` row rhythm, native-button content padding that keeps symbols away from the selected background edge, selected/unselected source-row font weight separation, compact symbols and type, true all-note counts from a shared library snapshot, restrained count color, warm selected-row tint, quieter hover feedback, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, persisted folder and folder/tag-section disclosure states that follow folder rename/delete lifecycles, a Notes-like recently-deleted row below the folder group, toolbar show/hide behavior, cached drag-target validation, focus-stable Up/Down navigation across visible source rows, Left/Right expand-collapse and parent-child navigation, and one native field-editor-backed inline create/rename control with default-name replacement, Return commit, Escape cancel, focus-loss commit, error recovery, and IME composition. The non-core call-recordings source is intentionally omitted; deeper hierarchy visual tuning remains open.
- Note-list rows show Notes-like three-line metadata with title, date-plus-preview without duplicated weekday prefixes, a separate folder/tags row with a folder icon, bounded body previews, a darker Notes-like column, a true filesystem-backed all-notes scope, compact `48pt` recency groups and `76pt` note rows, text-aligned content spacing, asymmetric selected-card breathing room around the scrollbar, and a Notes-style list-options hierarchy with persistent edit-date/creation-date/title sorting plus optional date grouping. Creation metadata reuses the indexed file-signature read, and group headers plus visible row dates follow the active date basis. A persistent `Pinned` group stays above date groups and follows note rename/move/delete lifecycles; single- and multi-note pin/unpin actions, search-result relevance order preservation, shared-snapshot folder/tag filtering, centralized horizontal spacing, quieter selection/hover states, subtle row separators, Markdown file drag-out, drag-to-folder moves, multi-selection file actions, multi-note move/delete/copy/export, piled drag previews, attachment indicators, local image thumbnails, launch-safe shell-first hydration, keyboard browsing, and empty states are present, but deeper drag-and-drop polish remains open.
- Editor now uses a centered date-only header for loaded notes, explicit left-aligned note titles, debounced autosave, lighter save-progress copy, compact `30pt` Notes-like title typography, a `15pt` body reading size, restrained line rhythm, a Notes-like date-to-title vertical rhythm, and a `24pt` content inset close to the editor divider. Empty notes and no-selection states keep a quiet blank canvas instead of showing a custom instructional overlay; deeper content-state tuning for complex notes remains open.
- Rich editor provides a stateful Notes-style `Aa` menu with idempotent H1/H2/H3/Body selection, checklist/bullet/numbered styles, and bold/italic/underline/strikethrough while dedicated toolbar actions retain toggle behavior. It renders Markdown tables as native editable AppKit grids while preserving portable Markdown on disk. Table workflows include Tab/Shift-Tab cell navigation, automatic row creation, row and column insertion/deletion, Command-Delete data-row removal, and table context menus. Links use a Notes-style window-attached Link To and Name sheet with destination validation, selected-text defaults, Return/Escape handling, editable labels and addresses, plus native open, Command-click, copy, and remove actions in both library and floating editors. Note-list previews strip table, link, emphasis, code, and list Markdown syntax. Attachment insertion, local image previews, openable non-image attachment chips, native Space-key/context-menu Quick Look, Finder multi-file paste, clipboard-image paste, HTML/RTF rich-text paste, selected-source Markdown export, and full-note Markdown copy are also present. Rich paste preserves headings, bold, italic, underline, strikethrough, links, bullets, numbering, and code-style runs while saving plain Markdown. Pasted assets use stable relative Markdown links under an internal `Attachments/yyyy/mm` directory that stays out of source navigation and note indexing; embedded rich-media paste and richer inline attachment preview types remain open.
...
- Source, folder, tag, recent-note, and empty-search navigation now paints from the loaded library snapshot immediately, then runs a coalesced cancellable filesystem-index validation off the main actor. Generation checks reject stale validation results, local saves update the snapshot immediately, and normal mutations cancel pending work so external Markdown changes remain visible without making repeated navigation block on a full file-signature scan.
- Visual QA has a repeatable active-window side-by-side harness with quiet source-loading rows, pointer-hover suppression before capture, explicit frontmost-app verification, true window-only capture normalization with bottom-to-top coordinate conversion for full-screen fallback crops, point/pixel/backing-scale/window-bounds/capture-mode metadata, labeled comparison output against supplied empty and content Apple Notes screenshots, focus-stable requested-note selection that avoids an editor caret in comparison captures, canonical `1080x720` sizing, reference-scale shell geometry, and a compact no-rim toolbar baseline. The accepted scale comparison is stored under `docs/visual-qa/compact-scale-comparison.png`.
