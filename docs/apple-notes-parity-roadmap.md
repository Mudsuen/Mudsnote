...
- Source list has compact group labels, a local `Call Recordings` smart source with a Notes-like phone-waveform icon derived from the existing note snapshot, tighter Notes-like default column geometry, stronger Notes-like typography and row height, brighter Notes-like unselected row text/icons, true all-note counts from a shared library snapshot, warm selected-row tint, quieter hover feedback, tightened top inset/disclosure/count geometry, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, session-local folder disclosure controls, Notes-like collapsible folder/tag sections, a Notes-like recently-deleted row below the folder group, toolbar show/hide behavior, and cached drag-target validation, but still needs deeper hierarchy visual tuning.
- Note-list rows show Notes-like compact three-line metadata with title, time-plus-preview, and a separate folder/tags row with a folder icon, bounded body previews, a darker Notes-like column, a true filesystem-backed all-notes scope, tighter Notes-like default column geometry, tightened title/snippet/meta typography and row height, shared-snapshot folder/tag filtering, centralized horizontal spacing, near-full-width selected highlights, low-contrast hover feedback, subtle row separators, native Markdown file drag-out, drag-to-folder moves with cached validation, multi-selection file actions, selection-count-aware action labels, multi-note move/delete/share/export, multi-note drag-to-folder moves with piled count previews, compact attachment indicators, local image thumbnails, launch-safe shell-first direct-open hydration with background first-note loading and immediate title/date shell, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still needs deeper drag-and-drop polish for edge states.
- Editor now uses a centered date-only header for loaded notes, explicit left-aligned note titles, debounced autosave, lighter save-progress copy, compact Notes-like title/body typography, wider body line rhythm, a Notes-like date-to-title vertical rhythm, and a Notes-like content inset closer to the editor divider, but still needs deeper side-by-side visual tuning.
