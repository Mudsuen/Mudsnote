...
- Source list has compact group labels, wider Notes-like default column geometry, stronger Notes-like typography and row height, true all-note counts from a shared library snapshot, warm selected-row tint, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, session-local disclosure controls, toolbar show/hide behavior, and cached drag-target validation, but still needs deeper hierarchy visual tuning.
...
- Visual QA has a repeatable active-window side-by-side harness with quiet source-loading rows, point/pixel/backing-scale metadata, and labeled comparison output against the supplied Apple Notes screenshot, but still needs per-iteration delta notes and closer visual tuning.
