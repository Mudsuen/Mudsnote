...
- Titlebar, list headers, default window proportion, and shared layout metrics now use a `940x630pt` shell close to the supplied `931x623pt` Apple Notes window. Default source/list columns remain `220/200pt`, matching the reference's asymmetric proportions; source/list widths, source visibility, and the whole workspace frame persist across launches. Restored frames are clamped onto the nearest visible display, while canonical visual QA ignores personal frame state. The version-5 migration resizes only exact previous-default `1080x680/720pt` frames around their existing center and preserves manually resized windows and panes.
