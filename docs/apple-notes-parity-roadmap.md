...
- Source list is a full-height native `NSSplitViewItem.sidebar` with an independent dark material and `24pt` rounded boundary covering the titlebar, traffic lights, Add Folder, and sidebar-toggle controls in expanded mode. Its rows begin below the titlebar safe area. When collapsed through the native split item, the entire region disappears and the content-list item becomes leftmost while the unified toolbar retains the sidebar control and current library title. Persisted widths, tracking separators, `222pt` rows, shared counts, deferred loading, disclosure, recently deleted, keyboard hierarchy navigation, drag targeting, and field-editor-backed inline create/rename remain intact. The non-core call-recordings source remains intentionally omitted; deeper hierarchy tuning remains open.
