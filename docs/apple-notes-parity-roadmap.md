...
- paste normalization for rich text and files (Finder files and clipboard images implemented; structured HTML paste remains open)
...
- Rich editor renders Markdown tables as native editable AppKit grids while preserving portable Markdown on disk. Table workflows include Tab/Shift-Tab cell navigation, automatic row creation, row and column insertion/deletion, Command-Delete data-row removal, and table context menus. Links support insertion plus native open, Command-click, address editing, copy, and remove actions in both library and floating editors. Note-list previews strip table, link, emphasis, code, and list Markdown syntax. Attachment insertion, local image previews, openable non-image attachment chips, native Space-key/context-menu Quick Look, Finder multi-file paste, clipboard-image paste, selected-source Markdown export, and full-note Markdown copy are also present. Pasted assets use stable relative Markdown links under an internal `Attachments/yyyy/mm` directory that stays out of source navigation and note indexing; structured rich-text paste and richer inline attachment preview types remain open.
