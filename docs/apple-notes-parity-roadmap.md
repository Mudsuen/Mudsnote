...
- Folder, tag, and Inbox source counts are aggregated from the in-memory note snapshot in one pass, including parent-folder rollups and case-insensitive per-note tag deduplication, instead of filtering the whole library once per visible source.
