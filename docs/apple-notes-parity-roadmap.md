...
- Note-list rows show bounded body previews, a darker Notes-like column, a true filesystem-backed all-notes scope, wider Notes-like default column geometry, stronger Notes-like title/snippet/meta typography and row height, shared-snapshot folder/tag filtering, centralized horizontal spacing, inset rounded selected cards, subtle hover feedback, subtle row separators, native Markdown file drag-out, drag-to-folder moves with cached validation, multi-selection file actions, selection-count-aware action labels, multi-note move/delete/share/export, multi-note drag-to-folder moves with piled count previews, compact attachment indicators, local image thumbnails, launch-safe shell-first direct-open hydration, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still needs deeper drag-and-drop polish for edge states.
...
- Search is in the top toolbar and supports current/all scoping with visible row highlights, editor-side match highlights, no-result feedback, multi-result stepping from the search field, keyboard loading into the editor, background indexing status during full-library hydration, and a prewarmed lightweight Markdown index with disk-backed snapshot persistence, but still needs deeper large-library cancellation/incremental refresh polish.
