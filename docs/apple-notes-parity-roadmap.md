...
- The editor-tools group should keep the earlier compact, borderless dark capsule treatment: `184x32`, `19pt` toolbar symbols, plain dark `NSView` fill, and no visible light rim or oversized custom buttons.
...
- Toolbar has a Notes-like expanded top chrome, split-view-tracking separators aligned to the source/list/editor pane boundaries, grouped editor-tools capsule for format/checklist/table/link/attachment actions with compact `184x32` plain dark-fill treatment and dimmed disabled states, borderless icon menu buttons for share/export and more actions, Markdown export/share affordances for system sharing, copying content, exporting one file, and sharing/exporting multiple selected Markdown files, a more-actions menu with selection-count-aware file action labels, a wider Notes-like search field, stateful source-list toggling, restrained toolbar symbol sizing, and context-aware disabled states, but still needs deeper toolbar icon-state tuning against Notes.
- Titlebar, list headers, default window proportion, and shared layout metrics are closer to Notes, with the default shell brought back to the earlier compact `1420x860` Notes-like canvas, screen-clamped presentation, and tighter source/list columns so the editor pane is not squeezed by oversized navigation panes.
- Source list has compact group labels, a local `Call Recordings` smart source with a Notes-like phone-waveform icon derived from the existing note snapshot, wider Notes-like default column geometry, stronger Notes-like typography and row height, brighter Notes-like unselected row text/icons, true all-note counts from a shared library snapshot, warm selected-row tint, quiet hover feedback, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, session-local folder disclosure controls, Notes-like collapsible folder/tag sections, a Notes-like recently-deleted row below the folder group, toolbar show/hide behavior, and cached drag-target validation, but still needs deeper hierarchy visual tuning.
- Note-list rows show Notes-like compact three-line metadata with title, time-plus-preview, and a separate folder/tags row with a folder icon, bounded body previews, a darker Notes-like column, a true filesystem-backed all-notes scope, wider Notes-like default column geometry, tightened title/snippet/meta typography and row height, shared-snapshot folder/tag filtering, centralized horizontal spacing, inset rounded selected cards, low-contrast hover feedback, subtle row separators, native Markdown file drag-out, drag-to-folder moves with cached validation, multi-selection file actions, selection-count-aware action labels, multi-note move/delete/share/export, multi-note drag-to-folder moves with piled count previews, compact attachment indicators, local image thumbnails, launch-safe shell-first direct-open hydration with background first-note loading and immediate title/date shell, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still needs deeper drag-and-drop polish for edge states.
- Editor now uses a centered date-only header for loaded notes, explicit left-aligned note titles, debounced autosave, lighter save-progress copy, compact Notes-like title/body typography, wider body line rhythm, tighter date/title/body spacing, and a Notes-like content inset closer to the editor divider, but still needs deeper side-by-side visual tuning.
