...
- Toolbar has a Notes-like expanded top chrome, grouped editor-tools capsule for format/checklist/table/link/attachment actions, Markdown export/share affordances for system sharing, copying content, exporting one file, and sharing/exporting multiple selected Markdown files, a more-actions menu with selection-count-aware file action labels, a wider Notes-like search field, stateful source-list toggling, larger toolbar symbol sizing, and context-aware disabled states, but still needs deeper toolbar spacing/icon-state tuning against Notes.
...
- Editor now uses a centered date-only header for loaded notes, explicit left-aligned note titles, debounced autosave, lighter save-progress copy, larger Notes-like title/body typography, and more Notes-like top/date/title/body rhythm, but still needs side-by-side visual tuning.
...
- Search is in the top toolbar and supports current/all scoping with visible row highlights, editor-side match highlights, no-result feedback, multi-result stepping from the search field, keyboard loading into the editor, debounced typing reloads with immediate keyboard-command flush, search reloads that skip unrelated source-count refreshes, background indexing status during full-library hydration, and a prewarmed lightweight Markdown index with disk-backed snapshot persistence, but still needs deeper large-library cancellation/incremental refresh polish.
