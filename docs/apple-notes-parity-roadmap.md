...
- Editor now uses a centered date-only header for loaded notes, debounced autosave, lighter save-progress copy, and more Notes-like top/date/title/body rhythm, but still needs side-by-side visual tuning.
...
- Search is in the top toolbar and supports current/all scoping with visible row highlights, no-result feedback, multi-result stepping from the search field, keyboard loading into the editor, and a prewarmed lightweight Markdown index with disk-backed snapshot persistence, but still needs match highlighting inside the editor and deeper large-library visual loading states.
