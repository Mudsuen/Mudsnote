...
- Toolbar has editing tools, a Markdown export/share affordance, a more-actions menu, stateful source-list toggling, and context-aware disabled states, but still needs side-by-side visual tuning and richer export destinations.
- Titlebar, list headers, default window proportion, and shared layout metrics are closer to Notes, with a larger first-launch desktop window and screen-clamped presentation, but still need exact toolbar balance tuning.
