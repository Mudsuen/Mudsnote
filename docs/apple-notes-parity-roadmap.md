...
- Toolbar has a Notes-like grouped editor-tools capsule for format/checklist/table/link/attachment actions, Markdown export/share affordances for system sharing, copying content, exporting one file, and sharing/exporting multiple selected Markdown files, a more-actions menu with selection-count-aware file action labels, a wider Notes-like search field, stateful source-list toggling, and context-aware disabled states, but still needs deeper toolbar spacing/icon-state tuning against Notes.
- Titlebar, list headers, default window proportion, and shared layout metrics are closer to Notes, with a larger first-launch desktop editor window, screen-clamped presentation, and stronger toolbar/editor spacing metrics.
- Source list has compact group labels, wider Notes-like default column geometry, stronger Notes-like typography and row height, true all-note counts from a shared library snapshot, warm selected-row tint, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, session-local folder disclosure controls, Notes-like collapsible folder/tag sections, toolbar show/hide behavior, and cached drag-target validation, but still needs deeper hierarchy visual tuning.
- Note-list rows show bounded body previews, a darker Notes-like column, a true filesystem-backed all-notes scope, wider Notes-like default column geometry, stronger Notes-like title/snippet/meta typography and row height, shared-snapshot folder/tag filtering, centralized horizontal spacing, inset rounded selected cards, subtle hover feedback, subtle row separators, native Markdown file drag-out, drag-to-folder moves with cached validation, multi-selection file actions, selection-count-aware action labels, multi-note move/delete/share/export, multi-note drag-to-folder moves with piled count previews, compact attachment indicators, local image thumbnails, launch-safe shell-first direct-open hydration with background first-note loading, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still needs deeper drag-and-drop polish for edge states.
- Editor now uses a centered date-only header for loaded notes, debounced autosave, lighter save-progress copy, larger Notes-like title/body typography, and more Notes-like top/date/title/body rhythm, but still needs side-by-side visual tuning.
...
- Search is in the top toolbar and supports current/all scoping with visible row highlights, editor-side match highlights, no-result feedback, multi-result stepping from the search field, keyboard loading into the editor, debounced typing reloads with immediate keyboard-command flush, background indexing status during full-library hydration, and a prewarmed lightweight Markdown index with disk-backed snapshot persistence, but still needs deeper large-library cancellation/incremental refresh polish.
- Visual QA has a repeatable active-window side-by-side harness with quiet source-loading rows, point/pixel/backing-scale/window-bounds metadata, and labeled comparison output against the supplied Apple Notes screenshot, but still needs per-iteration delta notes and closer visual tuning.
