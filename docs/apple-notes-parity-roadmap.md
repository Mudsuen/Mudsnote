...
- Rich editor renders Markdown tables as native editable AppKit grids while preserving portable Markdown on disk. Table workflows include Tab/Shift-Tab cell navigation, automatic row creation, row and column insertion/deletion, Command-Delete data-row removal, and table context menus. Table note-list previews are plain text instead of raw pipe syntax. Link and attachment insertion, local image previews, openable non-image attachment chips, native Space-key/context-menu Quick Look, selected-source Markdown export, and full-note Markdown copy are also present; richer inline attachment preview types remain open.
