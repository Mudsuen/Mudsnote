...
- Source list has compact group labels, tighter Notes-like row density, counts, warm selected-row tint, launch-safe root folder rows, deferred full folder loading, loading/empty state rows, tag rows after shell visibility, session-local disclosure controls, and toolbar show/hide behavior, but still needs side-by-side spacing checks and deeper hierarchy visual tuning.
- Note-list rows show bounded body previews, a darker Notes-like column, centralized horizontal spacing, inset rounded selected cards, subtle hover feedback, subtle row separators, native Markdown file drag-out, drag-to-folder moves, compact attachment indicators, local image thumbnails, tighter row density, shell-first direct-open hydration, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still need side-by-side spacing tuning and richer multi-item drag polish.
