...
- Note-list rows show bounded body previews, a darker Notes-like column, inset rounded selected cards, subtle hover feedback, native Markdown file drag-out, compact attachment indicators, local image thumbnails, tighter row density, shell-first direct-open hydration, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still need side-by-side spacing tuning and drag-to-folder move interactions.
