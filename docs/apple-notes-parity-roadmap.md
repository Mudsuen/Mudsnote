...
- Toolbar has editing tools, Markdown export/share affordances for system sharing, copying content, exporting one file, and sharing/exporting multiple selected Markdown files, a more-actions menu with selection-count-aware file action labels, a wider Notes-like search field, stateful source-list toggling, and context-aware disabled states, but still needs deeper toolbar visual tuning against Notes.
