...
- Editor now uses a centered date-only header for loaded notes, explicit left-aligned note titles, debounced autosave, lighter save-progress copy, compact `30pt` Notes-like title typography, a `15pt` body reading size, restrained line rhythm, a Notes-like date-to-title vertical rhythm, and a `24pt` content inset close to the editor divider, but still needs deeper content-state tuning for complex notes.
