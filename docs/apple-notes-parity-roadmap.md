...
- Note-list rows show bounded body previews, a darker Notes-like column, a true filesystem-backed all-notes scope, wider Notes-like default column geometry, stronger Notes-like title/snippet/meta typography, shared-snapshot folder/tag filtering, centralized horizontal spacing, inset rounded selected cards, subtle hover feedback, subtle row separators, native Markdown file drag-out, drag-to-folder moves with cached validation, multi-selection file actions, selection-count-aware action labels, multi-note move/delete/share/export, compact attachment indicators, local image thumbnails, launch-safe shell-first direct-open hydration, keyboard open/delete and Up/Down note browsing, and empty/no-result/trash feedback, but still need richer multi-item drag polish.
