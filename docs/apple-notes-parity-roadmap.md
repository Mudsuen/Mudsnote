...
- Editor now uses a centered date-only header for loaded notes and debounced autosave for editable library notes, but still needs exact title/body vertical spacing, more subtle save-state copy, and side-by-side visual tuning.
...
- Search is in the top toolbar and supports current/all scoping with visible row highlights, no-result feedback, and bidirectional keyboard transitions from search to result list/editor, but still lacks indexing and multi-result stepping inside the search field.
