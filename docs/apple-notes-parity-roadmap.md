...
- Rich editor has Markdown-level table/link/attachment insertion from the library toolbar, note-list image thumbnails, in-editor previews for local image references, non-image local attachment chips, and selected-source Markdown export, but still lacks full rich table editing and richer attachment actions/metadata.
