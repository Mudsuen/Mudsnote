...
- Toolbar has a Notes-like expanded top chrome, split-view-tracking separators aligned to the source/list/editor pane boundaries, grouped editor-tools capsule for format/checklist/table/link/attachment actions with borderless dark-fill treatment and dimmed disabled states, Markdown export/share affordances for system sharing, copying content, exporting one file, and sharing/exporting multiple selected Markdown files, a more-actions menu with selection-count-aware file action labels, a wider Notes-like search field, stateful source-list toggling, larger toolbar symbol sizing, and context-aware disabled states, but still needs deeper toolbar icon-state tuning against Notes.
...
- Visual QA has a repeatable active-window side-by-side harness with quiet source-loading rows, pointer-hover suppression before capture, explicit frontmost-app verification before screenshot capture, point/pixel/backing-scale/window-bounds metadata, labeled comparison output against the supplied Apple Notes screenshot, empty-note and content-note fixture selection modes, and tighter compact-shell geometry, but still needs per-iteration delta notes and closer toolbar/source-list visual tuning.
