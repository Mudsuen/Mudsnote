...
- Visual QA has a repeatable active-window side-by-side harness with quiet source-loading rows, pointer-hover suppression before capture, point/pixel/backing-scale/window-bounds metadata, labeled comparison output against the supplied Apple Notes screenshot, and wider Notes-like main columns/toolbar controls, but still needs per-iteration delta notes and closer toolbar/source-list visual tuning.
