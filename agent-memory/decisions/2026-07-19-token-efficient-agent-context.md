...
- Enforce 320-line/2,200-word default entry budgets, focused handoff/architecture
...
historical evidence on demand. At this checkpoint the entry set is 283 lines
(`-64.7%`) and 1,896 words instead of 6,903 (`-72.5%`). Architecture and workflow
drift now fail policy verification. Existing large files are not mechanically
split merely to reduce line counts; future changes have an explicit extraction
boundary.
